...
const SignIn = () => {
  ...
  const [signInWithEmailAndPassword, userEP, loadingEP, errorEP] =
  ...
  if (userEP || userG) {
    content = (
      ...
      >
        ...
        <strong className="font-bold">{userEP?.email || userG?.email}</strong>
      </div>
    );
  }
    ...
    content = (
      ...
      >
        <strong className="font-bold mr-1">
          {errorEP?.name || errorG?.name}
        </strong>
        <span className="block sm:inline">
          {errorEP?.message || errorG?.message}
        </span>
      </div>
    );
  }
